...
Un peu moins théorique que le reste des articles et un peu plus orienté vers l'implémentation, cet article liste un bon nombre de schémas cryptographiques reposant sur les réseaux euclidiens et fait la distinction entre ceux n'ayant pas de preuve de sécrité mais étant efficaces, et ceux prouvés comme surs mais étant moins efficaces voire inutilisables en pratique.
- l'article présente la focntion de hash du cours n°1. Elle a été inventée par Ajtai.
- Comme cette fonction n'est pas tres efficace à calculer, plusieurs propositions visent à ameliorer la complexité asymptotique ( première idée : matrices circulantes s'apparentant à de la multiplication sur Z[X]/(X^{n}-1) mais moins de preuves de sécurité. par contre, les matrices s'apparentant à de la multiplication sur Z[X]/(X^{n}+1) ont une preuve de sécurité reposant sur SVP sur les réseaux idéaux, et on peut utiliser la FFT pour optimiser le calcul de la fonction et la clé publique est moins lourde)
- présentation de l'algrithme d'encryption NTRU, non prouvé sûr mais jamais cassé ( l'algorithme utilise les réseaux idéaux, est un peu plus complexe à comprendre que d'habitude et est assez particulier, mais les clé privées et publiques sont assez légères (seulement un ou deux vecteurs de Z_{q}^{n} ) et les algorithmes d'encryptions et de décryptions sont plutot efficaces )
- Le papier détaille un des schémas de chiffrement les plus eficaces ayant une preuve de sécurité, il est basé sur LWE ( clé publique: matrice S choisie au hasard, clé publique: (A,P=AS+E) ou A est prise au hasard et E matrice de coef gaussien. Pour encrypter, (u=A^{T}a,c=P^{T}a+f(v)). Pour decrypter, f^{-1}(c-S^{T}u). f(x)=q/t*int(x) )
- Cet algorithme requiert beaucoup de parametres, mais une fois les bons parametres choisis, l'algorithme peut etre prouvé comme sécurisé et peut avoir des clés "messy" ( comme dans l'article n°3 )
...
Cet article traite de d'une preuve de robustesse de LWE dans un cas particulier; jusqu'ici, on avait supposé que le secret s du problème était tiré selon une distribution uniforme. Mais que se passe-t-il si la clé n'est pas tirée uniformément (cela peut être le cas en pratique)? L'article montre justement qu'avec une entropie suffisante, on peut conserver une preuve de sécurité.
...
- lemme 3 : si y est pris d'une gaussienne de paramètre \beta.q, alors |y|<=\beta.q avec proba accablante. De plus, la distance stat entre la gaussienne_{\beta q} et  la gaussienne_{\beta.q} + y est au plus |y|/(\beta q)
