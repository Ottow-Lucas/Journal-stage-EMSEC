...
- La plupart des réduction prouvant la difficulté de LWE entropique utilise une matrice A se décomposant en BC+F où chaque coef de F suit une gaussienne discrete. On obtient alors que le vecteur correspondant au produits scalaires est y = sA+e = sBC+sF+e
- l'approche de l'article n°8 consiste à "brouiller" sF+e en ayant un parametre enorme, pour que sF+e soit indistingable de la distribution de e. C'est un peu ce qui est fait dans l'artcle n°9 pour prouver que z* est indépendant de z ( t_{2} étant le vecteur permettant de brouiller)
- Ici, on suppose que la l'entropie minimale du secret conditionée par s+e_{1} est plus grande qu'une quantitée donnée ( cette donnée dit que la proba de retrouver s étant donné s+e_{1} est petite )
