...
- Dans l'article, on trouve le résultat suivant sur des gaussiennes CONTINUES : Si on prend F une matrice a coefficients entiers, \sigma > \sigma_{1}. Alors pour e vecteur de R^{m} est distribué selon une gaussienne d’écart-type \sigma, elle est de même loi que e_{1}F+e_{2} où F est fixé arbitrairement dans Z^{n.m}, e_{1} suit une distribution gaussienne d’écart-type \sigma_{1} et e_{2} suit une distribution gaussienne d’écart-type sqrt(\Sigma) où \Sigma=\sigma^{2}I-\sigma_{1}^{2}F^{T}F .
- 
